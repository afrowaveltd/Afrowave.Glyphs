...
public sealed class GlyphEditorViewModel : INotifyPropertyChanged
{
   ...
   public string HelpText => "Arrows/WASD move | Space toggle | C clear | I invert | F fill | Enter accept | Esc cancel | N save-next | K skip";
   ...
   private void RebuildText()
   {
      ...
      for(int y = 0; y < Bitmap.Size.Height; y++)
      {
         for(int x = 0; x < Bitmap.Size.Width; x++)
         {
            ...
            if(cur)
            {
               // Cursor cell: wrap with brackets, keep width stable.
               sb.Append('[');
               sb.Append(on ? '#' : '.');
               sb.Append(']');
            }
